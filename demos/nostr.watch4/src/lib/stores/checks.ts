...
import { eventsArray } from './events.js'; // Adjust the import path as necessary
...
export const checks = derived(eventsArray, ($eventsArray) => {
  return $eventsArray.map(transformCheck);
});
