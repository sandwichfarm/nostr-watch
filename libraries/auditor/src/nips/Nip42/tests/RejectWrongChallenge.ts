...
export class RejectWrongChallenge extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'RejectWrongChallenge'
  ...
  async prepare() {
    ...
    const template: EventTemplate = {
      ...
      tags: [['relay', this.socket.url], ['challenge', 'INVALID_WRONG_CHALLENGE_STRING']],
      content: ''
    }
  ...
  test({behavior}) {
    ...
    behavior.toBeOk(this.okReceived, 'relay responded with OK to wrong-challenge AUTH')
    behavior.toBeOk(
      !this.okAccepted,
      'relay rejected AUTH with wrong challenge (OK false)' + (this.okMessage ? ': ' + this.okMessage : '')
    )
  }
}

export default RejectWrongChallenge
