...
import { IEvent, IMonitor, IRelay, ICheck, INip11, ISsl, IGeocode } from './models/index'

import Db from './db';
import { NostrEvent } from '@base/models/NostrEvent';

export class DexieAdapter implements ICacheAdapter {

  private db: Dexie;
  private events!: Table<IEvent, string>;
  private monitors!: Table<IMonitor, string>;
  private relays!: Table<IRelay, string>;
  private checks!: Table<ICheck, string>;
  private nip11s!: Table<INip11, string>;
  private ssls!: Table<ISsl, string>;
  private geocodes!: Table<IGeocode, string>;
  private geohashes!: Table<any, string>;

  public worker: Worker | SharedWorker;

  constructor(dbName: string = 'Relays') {
    const { db } = Db(dbName);
    this.db = db;
    this.worker = new SharedWorker('./workers/dexie.worker.ts', { type: 'module' });
  }

  bulkSetEvents( events: NostrEvent[] ): void {
    this.worker.postMessage({ type: 'bulkEvents', events });
  }
