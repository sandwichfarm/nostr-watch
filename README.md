...
| @nostrwatch/trawler         | agent    | Scrapes all of the known nostr-verse for relays, sanitizes and dedupes them.                                      | alpha  |     |        | ✓    |     | rewrite imminent                                      |
...
| ~@nostrwatch/schemata~        | library  | JSON-Schemas for low-level protocol JSON, note kinds, protocol messages and NIP-11 information documents.        | alpha  | ✓   | ✓      |      |     | Moved to `@nostrability/schemata`                                                       |
| @nostrwatch/schemata-js-ajv | library  | Library that wraps `ajv` and `@nostrability/schemata` to simplify validation of nostr json payloads.                   | alpha  | ✓   | ✓      |      |     |                                                       |
...
_nodejs support is being phased out in favor of deno_
