// adapters/cache/DexieAdapter/src/index.ts

import Dexie, { Table } from 'dexie';
import { ICacheAdapter, GeohashOptions } from '../../../src/interfaces/ICacheAdapter';
...
export class DexieAdapter implements ICacheAdapter {
  private db: Dexie;
  private events!: Table<Event, string>;
  private monitors!: Table<Monitor, string>;
  private relays!: Table<Relay, string>;

  constructor(dbName: string = 'NIP66Cache') {
    ...
    this.db.version(1).stores({
      ...
      relays: '&id, eventId, monitorId, nips, isp, ip, geohash, countryCode, ownerPubkey, network, rttOpen, livenessStatus',
    });

    // Initialize table references
    this.events = this.db.table('events');
    this.monitors = this.db.table('monitors');
    this.relays = this.db.table('relays');
  }
