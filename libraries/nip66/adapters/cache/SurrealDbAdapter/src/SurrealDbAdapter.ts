...
import RelayDb from './db/connect';
...
class SurrealDbAdapter extends CacheAdapter implements ICacheAdapter {
    ...
    constructor(isMainThread: boolean = true) {
      super()
      RelayDb('idb').then( (db: Surreal) => this._db = db );
      if(isMainThread) {
        this.bindWorkerHandlers()
      }
    } 

    async addEvent(event: IEvent): Promise<void> { return }
    async putEvent(event: IEvent): Promise<void> { return }
