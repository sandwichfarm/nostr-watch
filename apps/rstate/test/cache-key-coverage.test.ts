...
const TOOL_FACTORIES: Record<string, () => any> = {
  ...
  'relays/list/detailed': () => createRelaysListDetailedTool(toolsContext),
  'relays/list/full': () => createRelaysListFullTool(toolsContext),
  'relays/state': () => createRelaysGetStateTool(toolsContext),
  'relays/trust': () => createRelaysTrustTool(toolsContext),
  'relays/trust/list': () => createRelaysTrustListTool(toolsContext),
  'relays/search': () => createRelaysSearchTool(toolsContext),
  'relays/search/detailed': () => createRelaysSearchDetailedTool(toolsContext),
  'relays/search/full': () => createRelaysSearchFullTool(toolsContext),
  'relays/nearby': () => createRelaysNearbyTool(toolsContext),
  'relays/nearby/detailed': () => createRelaysNearbyDetailedTool(toolsContext),
  'relays/nearby/full': () => createRelaysNearbyFullTool(toolsContext),
  'relays/bbox': () => createRelaysBboxTool(toolsContext),
  'relays/bbox/detailed': () => createRelaysBboxDetailedTool(toolsContext),
  'relays/bbox/full': () => createRelaysBboxFullTool(toolsContext),
  'relays/labels': () => createRelaysGetLabelsTool(toolsContext),
  'relays/labels/list': () => createRelaysListLabelsTool(toolsContext),
  'relays/by/label': () => createRelaysByLabelTool(toolsContext),
  'relays/by/label/detailed': () => createRelaysByLabelDetailedTool(toolsContext),
  'relays/by/label/full': () => createRelaysByLabelFullTool(toolsContext),
  'relays/by/software': () => createRelaysBySoftwareTool(toolsContext),
  'relays/by/network': () => createRelaysByNetworkTool(toolsContext),
  'relays/by/nip': () => createRelaysByNipTool(toolsContext),
  'relays/by/country': () => createRelaysByCountryTool(toolsContext),
  ...
  'relays/dead': () => createRelaysDeadTool(toolsContext),
}
...
describe('Layer 2: Behavioral Divergence — different params produce different cache keys', () => {
  ...
  for (const toolName of paginatedTools) {
    it(`${toolName}: limit/offset vary cache key`, () => {
      ...
      const baseParams = toolName.startsWith('relays/by/label')
  ...
  const shapeGroups = [
    { tools: ['relays/list', 'relays/list/detailed', 'relays/list/full'], params: { limit: 1, offset: 0 } },
    { tools: ['relays/search', 'relays/search/detailed', 'relays/search/full'], params: { network: 'clearnet', limit: 1, offset: 0 } },
    { tools: ['relays/nearby', 'relays/nearby/detailed', 'relays/nearby/full'], params: { lat: 37, lon: -122, radius: 100 } },
    { tools: ['relays/bbox', 'relays/bbox/detailed', 'relays/bbox/full'], params: { sw: { lat: 32, lon: -125 }, ne: { lat: 42, lon: -120 }, limit: 10 } },
    { tools: ['relays/by/label', 'relays/by/label/detailed', 'relays/by/label/full'], params: { namespace: 'nip32.geo', value: 'US', limit: 1, offset: 0 } },
  ]

  for (const group of shapeGroups) {
    it(`${group.tools.join(', ')}: split output shapes use distinct cache keys`, () => {
      const keys = group.tools.map((toolName) => getCacheConfig(toolName)!.cacheKeyFn!(group.params))
      expect(new Set(keys).size).toBe(keys.length)
    })
  }

  it('relays/by/nip: minSupport varies cache key', () => {
    const config = getCacheConfig('relays/by/nip')!
  ...
  it('relays/by/software: family varies cache key', () => {
    const config = getCacheConfig('relays/by/software')!
    const keyAll = config.cacheKeyFn({})
    const keyStrfry = config.cacheKeyFn({ family: 'strfry' })
    expect(keyAll).not.toBe(keyStrfry)
  })

  it('relays/by/software: different families produce different cache keys', () => {
    const config = getCacheConfig('relays/by/software')!
    const keyStrfry = config.cacheKeyFn({ family: 'strfry' })
...
describe('Layer 3: Integration — no cache poisoning through ToolRegistry pipeline', () => {
  ...
  it('relays/list: page 1 and page 2 return different results (no cache poisoning)', async () => {
    ...
    // They should be DIFFERENT relay URLs (not cache-poisoned)
    expect(result1.relays[0]).not.toBe(result2.relays[0])
  })

  it('relays/list/full vs relays/list return different structures', async () => {
    const resultFull = await registry.executeTool('relays/list/full', { limit: 1, offset: 0 })
    const resultSimple = await registry.executeTool('relays/list', { limit: 1, offset: 0 })
  ...
  it('relays/by/software: with family filter vs without returns different groups', async () => {
    const resultAll = await registry.executeTool('relays/by/software', {})
    const resultStrfry = await registry.executeTool('relays/by/software', { family: 'strfry' })
  ...
  it('relays/by/nip: different minSupport thresholds return different counts', async () => {
    const resultLow = await registry.executeTool('relays/by/nip', { minSupport: 0.0 })
    const resultHigh = await registry.executeTool('relays/by/nip', { minSupport: 1.0 })
