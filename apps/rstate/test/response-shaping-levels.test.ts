...
describe('Three-Level Response Shaping', () => {
  ...
  describe('Backward Compatibility: Legacy "compact" Parameter', () => {
    let server: RestServer
    let app: any
    let core: ReturnType<typeof initStateCore>
    ...
    describe('Legacy format="compact" (string)', () => {
      it('should map format=compact to detailed and emit deprecation header', async () => {
        const response = await app.inject({
          method: 'GET',
          url: '/relays?format=compact',
        })
    ...
    describe('Legacy compact=true (boolean)', () => {
      it('should map compact=true to detailed and emit deprecation header', async () => {
        const response = await app.inject({
          method: 'GET',
          url: '/relays?compact=true',
        })
    ...
    describe('Legacy compact=false (boolean)', () => {
      it('should map compact=false to full and emit deprecation header', async () => {
        const response = await app.inject({
          method: 'GET',
          url: '/relays?compact=false',
        })
    ...
    describe('POST /relays/search with legacy compact param', () => {
      it('should handle compact=true in request body', async () => {
        const response = await app.inject({
          method: 'POST',
          url: '/relays/search',
          payload: {
      ...
      it('should handle compact=false in request body', async () => {
        const response = await app.inject({
          method: 'POST',
          url: '/relays/search',
          payload: {
    ...
    describe('Format parameter takes precedence over compact', () => {
      it('should use format parameter when both are provided', async () => {
        const response = await app.inject({
          method: 'GET',
          url: '/relays?format=simple&compact=false',
        })

        expect(response.statusCode).toBe(200)
        const body = JSON.parse(response.body)
        // format=simple should win, returning string array
        if (body.relays.length > 0) {
          expect(typeof body.relays[0]).toBe('string')
        }
      })
