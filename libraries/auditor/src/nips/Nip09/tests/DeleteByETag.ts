...
export class DeleteByETag extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'DeleteByETag'
  ...
  async prepare() {
    ...
    const testEvent = signTestEvent(
      {kind: 1, created_at: Math.floor(Date.now() / 1000), tags: [], content: 'auditor-nip09-test'},
  ...
  test({behavior}) {
    ...
    behavior.toBeOk(!this.eventReturnedAfterDeletion, 'relay did not serve event after kind-5 e-tag deletion')
  }
}

export default DeleteByETag
