  ...
  return {
    debug: (msg: string) => {
      if (instanceLogLevel <= LogLevel.DEBUG) {
        console.debug(chalk.blue(`[${timestamp()}] [DEBUG] [${moduleName}] ${msg}`));
      }
    },
    info: (msg: string) => {
      if (instanceLogLevel <= LogLevel.INFO) {
        console.info(chalk.green(`[${timestamp()}] [INFO] [${moduleName}] ${msg}`));
      }
    },
    warn: (msg: string) => {
      if (instanceLogLevel <= LogLevel.WARN) {
        console.warn(chalk.yellow(`[${timestamp()}] [WARN] [${moduleName}] ${msg}`));
      }
    },
    error: (msg: string) => {
      if (instanceLogLevel <= LogLevel.ERROR) {
        console.error(chalk.red(`[${timestamp()}] [ERROR] [${moduleName}] ${msg}`));
      }
    },
