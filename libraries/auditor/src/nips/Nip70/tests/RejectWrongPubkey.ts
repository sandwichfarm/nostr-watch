...
export class RejectWrongPubkey extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'RejectWrongPubkey'

  completeOn: CompleteOnTypeArray = ['off']

  private phase: 'awaiting-auth-ok' | 'awaiting-event-ok' | 'done' = 'awaiting-auth-ok'
  private authKeypair: TestKeypair | null = null
  private eventKeypair: TestKeypair | null = null
  ...
  async prepare() {
    ...
    // Two separate keypairs: auth with authKeypair, event from eventKeypair
    this.authKeypair = generateTestKeypair()
    this.eventKeypair = generateTestKeypair()

    const authEvent = signAuthEvent(this.socket.url, challenge, this.authKeypair.secretKey)
  ...
  _onMessageOk(message: RelayOkMessage): boolean {
    if (this.phase === 'awaiting-auth-ok' && message[1] === this.authEventId) {
      ...
      // AUTH accepted — send protected event signed by a DIFFERENT keypair
      ...
      const protectedEvent = signTestEvent(
        {
          ...
          tags: [['-']],
          content: 'auditor-nip70-wrong-pubkey-test'
        },
        this.eventKeypair!.secretKey
      )
  ...
  test({behavior}) {
    behavior.toBeOk(this.authOkAccepted, 'AUTH handshake succeeded')
    behavior.toBeOk(!this.eventOkAccepted, 'relay rejected protected event when authenticated pubkey does not match event author')
  }
}

export default RejectWrongPubkey
