<template>
  <span v-if="this.store.jobs.getActiveSlug === slug" class="text-inherit">
    <span class="text-inherit">
      <span
        v-if="!store.jobs.isJobActive(this.slug) && !isSingle"
        class="hidden text-inherit"
      >
        checked {{ timeSinceRefresh }} ago
      </span>
      <span
        v-if="store.jobs.isJobActive(this.slug) && !isSingle"
        class="italic text-inherit ml-2 inline-block"
      >
        <svg
          class="animate-spin mr-1 -mt-0.5 h-4 w-5 inline"
        ...
        {{ this.store.jobs.getProcessed(this.slug).length }}/{{
          this.relays.length
        }}
        loaded
      </span>
    ...
    <span
      class="text-inherit mr-2 ml-2 hidden lg:inline"
      v-if="!store.jobs.isJobActive(this.slug)"
      >-</span
    >
    <span
      class="text-inherit mr-2 ml-2"
...
const localMethods = {
  LoadSeed(force, single) {
    ...
    //   return
    if (!this.isExpired(this.slug, 15 * 60 * 1000) && !force) return
  ...
  async LoadSeedJob(single) {
    ...
    const relays = this.relays.filter(
      relay => !this.store.jobs.isProcessed(this.slug, relay)
    )
          ...
          .on('event', async (relay, sub_id, event) => {
            if (subid === sub_id) {
              ...
              let topics = event?.tags
                ...
                .map(topicTag => [topicTag[1]?.toLowerCase(), topicTag[2]])
              ...
              const result = {
                ...
                check: {
                  ...
                  read: false,
                  write: false,
                  latency: data?.latency[this.store.prefs.region]?.final
                    ? true
                    : false,
                  averageLatency: data?.latency[this.store.prefs.region]
                    ?.average
                    ? true
                    : false
                }
              ...
              if (data?.info) result.info = data.info
              ...
              if (event?.tags) {
                ...
                result.check.connect =
                  connect.length && connect[0][1] === 'true' ? true : false
                result.check.read =
                  read.length && read[0][1] === 'true' ? true : false
                result.check.write =
                  write.length && write[0][1] === 'true' ? true : false
              }
              ...
              if (
                result?.info?.limitation?.payment_required &&
                !this.isLoggedIn
              ) {
              ...
              if (this.store.jobs.isProcessed(this.slug, relay)) return
...
export default defineComponent({
  ...
  unmounted() {
    clearInterval(this.interval)
    if (this.pool) this.closePool(this.pool)
  },
  beforeMount() {
  ...
  mounted() {
    if (this.isSingle) {
      ...
      if (this.store.jobs.isJobActive(this.slug)) this.LoadSeed(true)
      else this.LoadSeed()
    }

    if (!this.store.prefs.clientSideProcessing) this.setRefreshInterval()
  },
  updated() {},
  computed: Object.assign(SharedComputed, {
    timeSinceRefresh() {
      return (
        this.timeSince(this.store.jobs.getLastUpdate(this.slug)) || Date.now()
      )
    }
  }),
