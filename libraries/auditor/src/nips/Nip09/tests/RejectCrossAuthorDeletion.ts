...
export class RejectCrossAuthorDeletion extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'RejectCrossAuthorDeletion'

  completeOn: CompleteOnTypeArray = ['off']

  private keypairA: TestKeypair | null = null
  private keypairB: TestKeypair | null = null
  ...
  private eventStillExists = false
  ...
  async prepare() {
    this.keypairA = generateTestKeypair()
    this.keypairB = generateTestKeypair()
    const testEvent = signTestEvent(
      {kind: 1, created_at: Math.floor(Date.now() / 1000), tags: [], content: 'auditor-nip09-cross-author-test'},
      this.keypairA.secretKey
    )
  ...
  test({behavior}) {
    behavior.toBeOk(this.publishOkAccepted, 'relay accepted test event')
    behavior.toBeOk(this.eventStillExists, 'relay correctly ignored deletion from non-author keypair')
  }
}

export default RejectCrossAuthorDeletion
