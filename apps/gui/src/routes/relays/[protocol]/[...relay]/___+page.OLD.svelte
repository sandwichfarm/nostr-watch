...
<div class="hidden space-y-4 py-10 px-4 pb-16 md:block">
      ...
      <Tabs.Root
        ...
        <Tabs.List 
          ...
          <Tabs.Trigger 
            disabled={!$nip11} 
            value="nip11" 
            on:click={() => activeTab.set('nip11')}
            >
              NIP-11
            </Tabs.Trigger>

          <Tabs.Trigger 
            disabled={!(operatorPubkey && $operatorRelays?.relays?.length)} 
            value="operator-feed" 
            on:click={() => activeTab.set('operator-feed')}
            >
              Operator Feed
            </Tabs.Trigger>
...
<style lang="postcss">
    ...
    .tab {
        /* @apply text-left text-lg block w-full py-2 px-4 cursor-pointer; */
    }
