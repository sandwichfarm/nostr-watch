...
export default class Base {
  ...
  private cb: Record<string, Function>;
  private current: string | null;
  private previous: string | null;
  private hard_fail: boolean;
  private SAMPLE_EVENT: typeof SAMPLE_EVENT;
  private checks: string[];
  private checksRequested: string[];
  private checksIgnoreOutput: string[];
  private checksCustom: Record<string, any>;
  private adaptersInitialized: boolean;
  private adapters: Record<string, any>;
  private adaptersValid: string[];
  private url: string;
  private config: ConfigInterface;
  private results: ResultInterface;
  private session: SessionHelper;
  private timeouts: TimeoutHelper;
  private latency: LatencyHelper;
  private promises: DeferredWrapper;
  private logger: Logger;
  private count: Counter;

  constructor(url: string, config: Partial<ConfigInterface> = {}) {
    this.url = url;
    this._init(config);
    this.logger.debug(`constructor(${url}, ${JSON.stringify(this.config)})`);
  }

  private _init(config: Partial<ConfigInterface>): void {
    this._init_defaults();
  ...
  private _init_instances(config: Partial<ConfigInterface>): void {
  ...
  private order_requested_checks(): void {
   ...
   */
  async check(keys: string | string[], headers: boolean = true): Promise<any> {
    this.keys = keys;
    let result;
   ...
   */
  async _check(key: string): Promise<any> {
   ...
   */
  private maybe_timeout(key: string): Function {
   ...
   */
  async finish(key: string, data: any = {}): Promise<void> {
   ...
   */
  produce_result(key: string, data: any = {}): Object {
    const result: any = {};
   ...
   */
  keyFromSubid(subid: string): string {
  ...
  unsubscribe(subid: string): void {
    if (!this.isConnected())
      return;
    this.maybeExecuteAdapterMethod('websocket', 'unsubscribe', () => this.ws.send(['CLOSE', subid]), subid);
  }
  ...
  close(key = ""): void {
    ...
    this.maybeExecuteAdapterMethod('websocket', 'close', () => this.ws.close());
  ...
  terminate(key = ""): void {
    ...
    this.maybeExecuteAdapterMethod('websocket', 'terminate', () => this.ws.terminate());
  }
  ...
  */
   ...
   * @private
   */
  websocket_hard_fail(err: any): void {
   ...
   * @param {string} [session] - The session to clear tracked data for
   */
  maybeExecuteAdapterMethod(adapter: string, methodName: string, altFn: Function = () => {}, ...args: any[]): any {
   ...
   */
  async addDeferred(key: string, cb: Function = () => {}): Promise<any> {
   ...
   */
  async useAdapter(Adapter: Function): Promise<void> {
  ...
  /**
   * checksSupported
   * Returns all checks supported by class.
   * 
   * @public
   * @static
   * @returns {boolean}
