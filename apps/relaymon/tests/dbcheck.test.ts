...
Deno.test("Database Check Utility: should detect no duplicates in a clean database", async () => {
  ...
  try {
    ...
    // Run the check with the test database instance
    await checkDatabase({ dbInstance: db });

    // Verify database state - should still have 2 entries (no changes)
    const count = db.query(`SELECT COUNT(*) FROM relay_status`)[0][0];
    assertEquals(count, 2, "Should still have 2 relay entries");
  } finally {
    cleanupTestDB(db);
  }
});

Deno.test("Database Check Utility: should detect duplicated relay entries", async () => {
  const db = setupTestDB();
  try {
    ...
    // Verify database state - should still have duplicates (no fix applied)
    const count = db.query(`SELECT COUNT(*) FROM relay_status`)[0][0];
    assertEquals(count, 4, "Should still have 4 relay entries (duplicates not fixed)");

    // Verify we have duplicates for relay1
    const relay1Count = db.query(`SELECT COUNT(*) FROM relay_status WHERE url = ?`, ["wss://relay1.example.com"])[0][0];
    assertEquals(relay1Count, 2, "Should have 2 entries for relay1");
  } finally {
    cleanupTestDB(db);
  }
});

Deno.test("Database Check Utility: should fix duplicated relay entries with auto method", async () => {
  const db = setupTestDB();
  try {
    ...
    // Run the check with auto fix and the test database instance
    await checkDatabase({ fix: "auto", dbInstance: db });
...
Deno.test("Database Check Utility: should fix duplicated relay entries with backup method", async () => {
  const db = setupTestDB();
  try {
    // Set up the same corrupted database state as the previous test
    // Create a table without PRIMARY KEY constraint
    ...
    // Run the check with backup fix and the test database instance
    await checkDatabase({ fix: "backup", dbInstance: db });
