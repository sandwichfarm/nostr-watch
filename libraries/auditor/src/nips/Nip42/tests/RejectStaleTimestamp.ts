...
export class RejectStaleTimestamp extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'RejectStaleTimestamp'
  ...
  async prepare() {
    ...
    const template: EventTemplate = {
      ...
      tags: [['relay', this.socket.url], ['challenge', challenge]],
      content: ''
    }
  ...
  test({behavior}) {
    ...
    behavior.toBeOk(this.okReceived, 'relay responded with OK to stale AUTH')
    behavior.toBeOk(
      !this.okAccepted,
      'relay rejected AUTH with stale created_at (OK false)' + (this.okMessage ? ': ' + this.okMessage : '')
    )
  }
}

export default RejectStaleTimestamp
