...
const randomizeCase = (input: string): string => {
  return input
      .split('')
      .map(char => {
          const shouldUpperCase = Math.random() < 0.5;
          return shouldUpperCase ? char.toUpperCase() : char.toLowerCase();
      })
      .join('');
}

const invertCase = (input: string): string =>
  input
    .split('')
    .map(char =>
...
export class InvertedCaseSearch extends SuiteTest implements ISuiteTest {
  readonly slug: string = 'InvertedCaseSearch';
  ...
  get filters(): Nip50Filter[] {
    ...
    const search = invertCase(this.searchTermsToFilter[0])
...
export default InvertedCaseSearch;